...
class Users
{
  ...
  {
    global $db;
    $sql = "SELECT u.name, u.email, r.name as role, u.photo  from ecom_users u, ecom_roles r WHERE email = '{$_email}' AND password = '{$_password}'AND u.role_id = r.id";
    ...
    if ($res) {
      if ($res->num_rows > 0) {
        ...
        return ["success" => "Login successful", "user_data" => $result, "token" => $token];
      } else {
        return ["error" => "Invalid email or password"];
      }
    } else {
      return "Query failded:" . $db->error;
    }
  }
